...
int engine(int numassets, int numfactors, 
	   ...
	   double *V, double *F, int max_names, double *theta, char *paramsfilename);
...
int main(int argc, char **argv)
{
  ...
  if(argc == 3){
    retcode = readstructure(argv[2], &max_names, &theta, numassets, &paramsfilename);
    if(retcode) goto BACK;
  }

  retcode = engine(numassets, numfactors, ub, lb, mu, sigma2, V, F, max_names,
		   theta, paramsfilename);
	     ...
	     double **pV, double **pF)
{
  ...
  printf("-> %s\n", buffer);
  for(j = 0; j < numassets; j++){
	  fscanf(input,"%s", buffer); ub[j] = atof(buffer);
  } 

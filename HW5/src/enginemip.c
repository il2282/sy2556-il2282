	   ...
	   char *paramsfilename)
{
 ...
 /* Create initial model */
  ...
  for(j = numassets + numfactors; j < n; j++){
    names[j] = (char *)calloc(3, sizeof(char));
    if(names[j] == NULL){
		  retcode = 1; goto BACK;
    }
  ...
  for(j = 0; j < n; j++){
    ...
    if (j < numassets + numfactors) vartype[j] = GRB_CONTINUOUS;
  ...
  retcode = GRBwrite(model, "factorqpmip.lp");
  if (retcode) goto BACK;

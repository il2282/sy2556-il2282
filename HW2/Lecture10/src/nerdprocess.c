...
int writelp(double *tomatch, double r, int T, char *lpfilename, int nerdID);
int solverinteraction(double *tomatch, double *pvalue, double r, int T, int nerdID);
double drawnormal(void);

int processfile(char *filename, double *pfinalcashflow, int nerdID)
{
  int code = 0;
  char buffer[100];
  int T;
  ...
  fscanf(theflows,"%s",buffer);
  r = atof(buffer);
  fscanf(theflows,"%s",buffer); /* the word 'rate', which we ignore **/
  printf("T = %d r = %g\n", T, r);
  if( (T <= 0) ){
  ...
  code = solverinteraction(tomatch, pfinalcashflow, r, T, nerdID);
  if(code) goto OUT;
...
int solverinteraction(double *tomatch, double *pfinalcashflow, double r, int T, int nerdID)
{
  ...
  sprintf(LPfilename,"cash_%d.lp",nerdID);
  sprintf(hiddenfilename,"hidden_%d",nerdID);
  sprintf(nothiddenfilename,"nothidden_%d",nerdID);
  sprintf(gurobipylogname,"mygurobi_%d",nerdID);

  solvecode = writelp(tomatch, r, T, LPfilename, nerdID);
  ...
  sprintf(commandbuffer, "python gurobiwithnerd.py %s %s %s %d", 
...
int writelp(double *tomatch, double r, int T, char *lpfilename, int nerdID)
{
  ...
  for(t = 2; t <= T; t++){
   ...
   if(t < T) fprintf(LPfile,"x%d", t);
    if(t < T-2) fprintf(LPfile," + y%d", t);
    if(t >= 4){
          fprintf(LPfile," - 1.025 y%d", t-3);
    }
    if(t < T) fprintf(LPfile," - %g x%d + 1.007 z%d - z%d = %g\n", rplus1, t-1,t-1,t,-tomatch[t-1]); 

    if(t == T) fprintf(LPfile," - %g x%d + 1.007 z%d - v%d = %g\n", rplus1, t-1,t-1,T,-tomatch[t-1]);

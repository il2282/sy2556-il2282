...
int sendword(char *destination, char *word, int ID);
...
int main(int argc, char **argv)
{
  ...
  if(argc != 3){
    printf("master: usage is master workfile numnerds\n"); runcode = 1; goto BACK;
  }

  numnerds = atoi(argv[2]);
  ...
  for(j = 0; j < numnerds; j++){
    ...
    sprintf(masterfilename[j],"masterfile%d.txt", j);
    sprintf(nerdfilename[j],"nerdfile%d.txt", j);
  ...
  for(workindex = 0; workindex < initialN; workindex++){
    runcode = sendword(masterfilename[workindex], wordlist[workindex], workindex);
  ...
  for(;;){
	...
	if (does_it_exist(nerdfilename[j])){
	  ...
	  if(workindex < N){
	    runcode = sendword(masterfilename[j], wordlist[workindex], j);
	    if(runcode) goto BACK;
  ...
  for(j = 0; j < numnerds; j++){
    runcode = sendword(masterfilename[j], "x", j);
    if(runcode) goto BACK;
...
int sendword(char *tokenfilename, char *word, int ID)
{
  int sendcode = 0;
  FILE *out;

  printf("master: sending '%s' to nerd %d using token file %s\n", word, ID,
	 tokenfilename);
